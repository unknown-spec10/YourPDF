...
import { ArrowLeft, Layers, ArrowUp, ArrowDown, X, Download, RefreshCw, AlertTriangle, Plus, Eye } from 'lucide-react'
...
export default function MergePage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.error(err)
      alert('Failed to initialize merge task. Please check server connection.')
    } finally {
    ...
    <div className="mx-auto w-full max-w-3xl px-6 py-12 flex-1 flex flex-col justify-center">
      ...
      <div className="mb-8">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-muted mt-1">Combine multiple PDF files into one document in seconds.</p>
          </div>
        </div>
      </div>
        ...
        <AnimatePresence mode="wait">
          ...
          {files.length > 0 && !jobId && (
            ...
            >
              ...
              >
                {isSubmitting ? 'Initializing...' : `Merge ${files.length} PDFs`}
              </button>
            </motion.div>
          )}
            ...
            >
              {(isQueued || isProcessing) && (
                <div className="w-full max-w-md">
                  <ProgressBar
                    ...
                    message={jobStatus?.message || 'Merging PDF files...'}
                  />
                </div>
              )}
              ...
              {isDone && jobStatus?.result && (
                <div className="flex flex-col items-center gap-4">
                  ...
                  <div>
                    <h3 className="text-2xl font-bold">Merge Completed!</h3>
                    <p className="text-sm text-muted mt-1">Your combined PDF file is ready. Click below to download.</p>
                  </div>
              ...
              {isError && (
                <div className="flex flex-col items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted mt-1">{jobStatus?.message || 'Failed to merge PDF files.'}</p>
                  </div>
                  <button
                    onClick={handleReset}
      ...
      {isDone && jobStatus?.result && (
        <PdfPreviewModal
          ...
          fileName={files[0]?.name ? `${files[0].name.replace(/\.[^/.]+$/, "")}_merged.pdf` : "merged.pdf"}
        />
      )}
    </div>
  )
