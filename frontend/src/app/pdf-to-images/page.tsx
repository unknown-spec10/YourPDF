...
export default function PdfToImagesPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await axios.post(`${API_URL}/api/pdf-to-images`, formData, {
        headers: { 'Content-Type': 'multipart/form-data' },
      })
      setJobId(response.data.job_id)
    } catch (err: any) {
      console.error(err)
      alert(err.response?.data?.detail || 'Failed to initialize image conversion. Please check server connection.')
    } finally {
    ...
    <div className="mx-auto w-full max-w-3xl px-6 py-12 flex-1 flex flex-col justify-center">
      ...
      <div className="mb-8">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-3xl font-bold tracking-tight">PDF to Images</h1>
            <p className="text-muted mt-1">Convert your PDF pages into high-quality images (PNG, JPG).</p>
          </div>
        </div>
      </div>
        ...
        <AnimatePresence mode="wait">
          ...
          {file && !jobId && (
            ...
            >
              ...
              >
                {isSubmitting ? 'Initializing...' : 'Convert to Images'}
              </button>
            </motion.div>
          )}
              ...
              {(isQueued || isProcessing) && (
                <div className="w-full max-w-md">
                  <ProgressBar
                    ...
                    message={jobStatus?.message || 'Rasterizing PDF pages...'}
                  />
                </div>
              )}
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted mt-1">
                      {jobStatus.result.download_url.toLowerCase().endsWith('.zip')
                        ? 'Your ZIP archive is ready for download.'
