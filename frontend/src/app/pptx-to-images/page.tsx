...
import { ArrowLeft, FileImage, Settings, Download, RefreshCw, AlertTriangle } from 'lucide-react'
...
export default function PptxToImagesPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await axios.post(`${API_URL}/api/office/pptx-to-images`, formData, {
        headers: { 'Content-Type': 'multipart/form-data' },
      })
      setJobId(response.data.job_id)
    } catch (err: any) {
      console.error(err)
      alert(err.response?.data?.detail || 'Failed to initialize PowerPoint image conversion. Please check server connection.')
    } finally {
    ...
    <div className="mx-auto w-full max-w-3xl px-6 py-12 flex-1 flex flex-col justify-center">
      ...
      <div className="mb-8">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-3xl font-bold tracking-tight">PPT to Images</h1>
            <p className="text-muted mt-1">Convert PowerPoint slides (.pptx) into individual PNG or JPG images.</p>
          </div>
        </div>
      </div>
              ...
              {(isQueued || isProcessing) && (
                <div className="w-full max-w-md">
                  <ProgressBar
                    ...
                    message={jobStatus?.message || 'Rasterizing PowerPoint slides...'}
                  />
                </div>
              )}
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted mt-1">
                      {jobStatus.result.download_url.toLowerCase().endsWith('.zip')
                        ? 'Your ZIP archive containing slide images is ready.'
