...
import { ArrowLeft, File, Download, RefreshCw, AlertTriangle, Eye } from 'lucide-react'
...
export default function PptxToPdfPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await axios.post(`${API_URL}/api/office/pptx-to-pdf`, formData, {
        headers: { 'Content-Type': 'multipart/form-data' },
      })
      setJobId(response.data.job_id)
    } catch (err: any) {
      console.error(err)
      alert(err.response?.data?.detail || 'Failed to initialize PowerPoint conversion. Please check server connection.')
    } finally {
    ...
    <div className="mx-auto w-full max-w-3xl px-6 py-12 flex-1 flex flex-col justify-center">
      ...
      <div className="mb-8">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-3xl font-bold tracking-tight">PPT to PDF</h1>
            <p className="text-muted mt-1">Convert PowerPoint presentations (.pptx) into high-quality PDF files.</p>
          </div>
        </div>
      </div>
          ...
          {!file && !jobId && (
            ...
            >
              <DropZone
                ...
                placeholderText="Drop your PowerPoint presentation (.pptx) here"
              />
            </motion.div>
          )}
              ...
              {(isQueued || isProcessing) && (
                <div className="w-full max-w-md">
                  <ProgressBar
                    ...
                    message={jobStatus?.message || 'Converting slides to PDF...'}
                  />
                </div>
              )}
                ...
                <div className="flex flex-col items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted mt-1">
                      Your presentation PDF is ready. Click the button below to download.
                    </p>
                  </div>
      ...
      {isDone && jobStatus?.result && (
        <PdfPreviewModal
          ...
          fileName={file?.name ? `${file.name.replace(/\.[^/.]+$/, "")}.pdf` : "presentation.pdf"}
        />
      )}
    </div>
  )
